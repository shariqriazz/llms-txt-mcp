...
export async function summarizeContentFiles(
    ...
    // --- Initialize LLM Client (remains the same) ---
    let llmClient: any;
    // (LLM Client Initialization logic as before...)
    if (PIPELINE_LLM_PROVIDER === 'gemini') {
        if (!GEMINI_API_KEY) throw new McpError(ErrorCode.InvalidRequest, 'PIPELINE_LLM_PROVIDER is "gemini" but GEMINI_API_KEY not set.');
        const genAI = new GoogleGenerativeAI(GEMINI_API_KEY);
        const safetySettings = [ { category: HarmCategory.HARM_CATEGORY_HARASSMENT, threshold: HarmBlockThreshold.BLOCK_NONE }, { category: HarmCategory.HARM_CATEGORY_HATE_SPEECH, threshold: HarmBlockThreshold.BLOCK_NONE }, { category: HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT, threshold: HarmBlockThreshold.BLOCK_NONE }, { category: HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT, threshold: HarmBlockThreshold.BLOCK_NONE }, ];
        llmClient = genAI.getGenerativeModel({ model: PIPELINE_LLM_MODEL, safetySettings });
    } else if (PIPELINE_LLM_PROVIDER === 'ollama') {
        const ollamaConfig: { host?: string } = {}; if (OLLAMA_BASE_URL) ollamaConfig.host = OLLAMA_BASE_URL; llmClient = new Ollama(ollamaConfig); safeLog?.('info', `[${taskId}] Using Ollama provider with model ${PIPELINE_LLM_MODEL} at ${OLLAMA_BASE_URL || 'default'}`);
    } else if (PIPELINE_LLM_PROVIDER === 'openrouter') {
        if (!OPENROUTER_API_KEY) throw new McpError(ErrorCode.InvalidRequest, 'PIPELINE_LLM_PROVIDER is "openrouter" but OPENROUTER_API_KEY not set.'); llmClient = new OpenAI({ apiKey: OPENROUTER_API_KEY, baseURL: OPENROUTER_BASE_URL }); safeLog?.('info', `[${taskId}] Using OpenRouter provider with model ${PIPELINE_LLM_MODEL} at ${OPENROUTER_BASE_URL}`);
    } else if (PIPELINE_LLM_PROVIDER === 'chutes') {
        if (!CHUTES_API_KEY) throw new McpError(ErrorCode.InvalidRequest, 'PIPELINE_LLM_PROVIDER is "chutes" but CHUTES_API_KEY not set.'); llmClient = new OpenAI({ apiKey: CHUTES_API_KEY, baseURL: CHUTES_BASE_URL }); safeLog?.('info', `[${taskId}] Using Chutes provider with model ${PIPELINE_LLM_MODEL} at ${CHUTES_BASE_URL}`);
    } else { throw new McpError(ErrorCode.InvalidRequest, `Unsupported PIPELINE_LLM_PROVIDER: ${PIPELINE_LLM_PROVIDER}. Must be 'gemini', 'ollama', 'openrouter', or 'chutes'.`); }
    ...
    updateTaskDetails(taskId, `Synthesize Stage: Summarizing ${filesToProcess.length} content files (LLM Concurrency: ${LLM_CONCURRENCY})...`);
    ...
    const llmPromises = filesToProcess.map((filename, index) =>
        llmLimiter(async (): Promise<LlmResult> => {
            ...
            try {
                ...
                safeLog?.('debug', `[${taskId}] Synthesize Stage: Sending content from ${filename} to ${PIPELINE_LLM_PROVIDER}...`);

                let generatedSection = '';
                let llmErrorReason = `LLM (${PIPELINE_LLM_PROVIDER}) did not return valid content.`;
                ...
                if (PIPELINE_LLM_PROVIDER === 'gemini') {
                    ...
                    const llmResult = await llmClient.generate({ model: PIPELINE_LLM_MODEL, prompt: prompt, stream: false });
                    if (!llmResult || !llmResult.response) throw new Error(llmErrorReason);
                    generatedSection = llmResult.response;
                } else if (PIPELINE_LLM_PROVIDER === 'openrouter' || PIPELINE_LLM_PROVIDER === 'chutes') {
                    const llmResult = await llmClient.chat.completions.create({ model: PIPELINE_LLM_MODEL, messages: [{ role: "user", content: prompt }] });
                    if (!llmResult || !llmResult.choices || llmResult.choices.length === 0 || !llmResult.choices[0].message?.content) {
                        const fr = llmResult?.choices?.[0]?.finish_reason; if (fr && fr !== 'stop') llmErrorReason = `${PIPELINE_LLM_PROVIDER} generation finished unexpectedly: ${fr}`;
                        throw new Error(llmErrorReason);
                    }
                ...
                safeLog?.('debug', `[${taskId}] Synthesize Stage: Successfully generated section from ${filename} using ${PIPELINE_LLM_PROVIDER}.`);
                ...
                if (['openrouter', 'chutes'].includes(PIPELINE_LLM_PROVIDER) && llmError instanceof Error) {
